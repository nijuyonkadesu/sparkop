...
# 2 - reshape the data to fit the schema
bicycle = {
        "brand": "Velorim",
        "model": "Jigger",
        "price": 270,
...
# bicycles [ scheme, schema ... ]
# Tag is similar to full text fields
# query: @condition{ "new", "refubished" }
# 1 - the schema
schema = (
...
res = r.hset(name="test:1", mapping=bicycle)
rs = r.ft("idx:fine")
rs.search(Query("Paul @age:[30 40]"))
# 3 - create indexes
# ft = full text index name
user1 = {
    "name": "Paul John",
...
# Using a specific path to insert user3
# Inserting user3 at the specified path
r.json().set("user:3", "$.nested.structure.users", user3)
r.json().set("user:1", Path.root_path(), user1)

# Printing the modified structure
print(existing_structure)
index = r.ft("idx:bicycle")
index.create_index(
    schema,
